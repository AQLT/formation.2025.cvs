...
library(zoo)
library(tikzDevice)
outliers <- ts.union(ao(),ls(),tc(),so(), ramp(),tls(),pso())
outliers <- window(outliers, start = 1996, end = c(2003,12))
# outliers <- window(outliers, start = 1999, end = c(2002,12))
colnames(outliers) <- c("AO","LS","TC","SO","Ramp","TLS","PSO")


tikz("Cours/img/outliers.tex",width=4.8,height=2.7)
plot(zoo(outliers),
     main= "",
...
for(nom_col in colnames(outliers)){
    tikz(paste0("Cours/img/",nom_col,".tex"),width=1.3,height=0.6)
