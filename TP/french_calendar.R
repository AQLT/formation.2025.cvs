...
holidays(FR, start = "1990-01-01", length = 12)
...
td_reg <- calendar_td(FR, frequency = frequency, start = start, length = 12*40,
              groups = groups)
...
rjd3toolkit::td_f(log(sa), nyears = 8)$pv
rjd3toolkit::td_f(log(i), nyears = 8)$pv
rjd3toolkit::td_f(sa2, nyears = 8)
rjd3toolkit::td_f(i2, nyears = 8)
...
td_reg <- calendar_td(FR, frequency = frequency, start = start, length = 12*35,
...
groups <- c(1,2,3,4,5,6,0)
frequency <- 12
H <- calendar_td(FR, frequency, c(1980,1), 12*30, groups,
         contrasts = TRUE)
...
HC<-calendar_td(FR, 12, c(1980,1), 140, c(1,1,1,1,1,0,0), contrasts = F)
...
frequency <- 12
start <- c(2000,1)
wkd <- calendar_td(FR, frequency = frequency, start = start, length = 12*2,
...
frequency <- 12
start <- c(2013,1)
wkd <- calendar_td(FR, frequency = frequency, start = c(2013,1), length = 12,
           groups = groups, contrasts = T)
wkd <- ts(wkd, start = start, frequency = frequency)
wkd2 <- calendar_td(FR, frequency = frequency, start = c(2013,1), length = 12,
...
wkd <- calendar_td(FR, frequency = frequency, start = c(2013,1), length = 12*10,
           groups = groups, contrasts = T)
wkd <- ts(wkd, start = start, frequency = frequency)
for (mois in seq_len(nrow(lg_mean))){
...
groups = c(1, 1, 1, 1, 2, 0, 0)
con2 <- calendar_td(FR,4,c(2000,1),12,groups = groups,contrast = TRUE)
len2 <- calendar_td(FR, 4,c(2000,1),12,groups = groups,contrast = FALSE)
con[,1] - (len[,2] - 4/2*len[,1])
